...
if (!token) {
	console.error(
		"Missing SANITY_API_TOKEN. Create a token with write access at: https://sanity.io/manage"
	);
	process.exit(1);
}
...
const initialData = [
	...
	// -------------------------------------------------------------------------
	{
		...
		visible: true,
		sectionLabel: "Investment Framework",
		title: "V.O.L.T.",
		description:
	...
	// -------------------------------------------------------------------------
	{
		...
		displayLimit: 5,
		tableHeaders: [
			{ _key: "th1", label: "Name", align: "left" },
			{ _key: "th2", label: "Price Zone", align: "left" },
			{ _key: "th3", label: "Status", align: "left" },
			{ _key: "th4", label: "Entry Price", align: "left" },
			{ _key: "th5", label: "Exit Price", align: "left" },
			{ _key: "th6", label: "Gains", align: "left" },
			{ _key: "th7", label: "Duration", align: "left" },
			{ _key: "th8", label: "Research Report", align: "left" },
		],
		blurredRowsCount: 3,
		...
		items: [
			{
				...
				testimonial:
					'"Magnus Hathaway was there during the crash when others disappeared. The research reports helped me understand fundamentals, not just follow tips."',
			},
			{
				_key: "test2",
				name: "Name Surname",
				positionAndCompany: "Position, Company name",
				testimonial:
					'"I had money stuck in laggard stocks. The team helped me reallocate based on research. Now I invest with confidence, not fear."',
			},
			{
				_key: "test3",
				name: "Name Surname",
...
async function seedData() {
	...
	console.log(
		"║          SANITY DATA MIGRATION - Magnus Hathaway LP            ║"
	);
	console.log(
		"╚════════════════════════════════════════════════════════════════╝"
	);
